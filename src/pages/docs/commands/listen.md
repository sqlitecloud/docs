...
The optional TABLE flag specifies that you want to receive notification for a given table inside the current database.
LISTENING to a table means you'll receive notification about all the write operations in that table.
In the case of TABLE, the channel_name can be *, which means you'll start receiving notifications from all the tables inside the current database.
