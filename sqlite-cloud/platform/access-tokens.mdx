...
async function getSQLiteCloudToken(userId: string) {
  ...
  const res = await fetch("https://<your-project-id>.<your-hosted-zone>.gateway.sqlite.cloud/v2/tokens", {
    method: "POST",
    headers: {
...
```typescript
const res = await fetch("https://<your-project-id>.<your-hosted-zone>.gateway.sqlite.cloud/v2/weblite/sql", {
  method: "POST",
  headers: {
