...
5. **Query data**
    - There are 2 ways to query data.
        1. In your SQLite Cloud account dashboard, click on a Node, copy the Connection String, and paste it into the form's `Database Connection String` input. The expected string format is: `sqlitecloud://{host}:8860?apikey={apikey}`.
        ...
        2. An alternative Connection String format is: `sqlitecloud://{username}:{password}@{host}:8860/{database}`.
        ...
        - To get the host, click your Project in the dashboard and copy the hostname from the Connect panel. It looks like `myproject.myzone.sqlite.cloud`.
