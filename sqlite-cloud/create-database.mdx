...
The Weblite API does not accept a database file in the request body. Instead, it transfers the database from a URL you provide: make a POST request to the `/v2/weblite/<database-name>.sqlite` endpoint with a JSON body containing the `location` of the file. The URL must be reachable from the SQLite Cloud gateway, so a path on your own machine will not work; to upload a local file, use the Dashboard.
...
curl -X 'POST' \
  ...
  -d '{"location": "https://<url-of-your-database-file>"}'
```
...
const db = new Database('sqlitecloud://<your-project-id>.<your-hosted-zone>.sqlite.cloud:<your-host-port>?apikey=<your-api-key>')
