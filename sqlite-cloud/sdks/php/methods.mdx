...
SQLiteCloudClient.connectWithString('sqlitecloud://myproject.myzone.sqlite.cloud:8860?apikey=myapikey')
```
...
try {
	if ($sqlitecloud->connectWithString('sqlitecloud://myproject.myzone.sqlite.cloud:8860/mydatabase?apikey=myapikey') == false) {
...
try {
	if ($sqlitecloud->connect('myproject.myzone.sqlite.cloud', 8860) == false) {
		$msg = $sqlitecloud->errmsg;
		return $msg;
	}
} catch (Exception $e) {
...
Submits a command to the server and waits for the result. The command can be any SQLite statement or any built-in [SQLite Cloud command](/docs/server-side-commands).
...
$sqlitecloud = new SQLiteCloudClient();
$sqlitecloud->connectWithString('sqlitecloud://myproject.myzone.sqlite.cloud:8860/mydatabase?apikey=myapikey')
