---
title: "SQCloudRowsetValue"
description: SQCloud Rowset C/C++ Interface SQCloudRowsetValue
category: sdks
status: publish
---
...
### Description
If the result of the function [SQCloudResultType](/docs/sqlite-cloud/sdks/c/sqcloudresulttype) is RESULT_ROWSET then use this function to retrieve a pointer and a length for a rowset item identified by a given row and column index.
...
int main (int argc, const char * argv[]) {
    ...
    SQCloudConnection *conn = SQCloudConnect("myproject.myzone.sqlite.cloud", SQCLOUD_DEFAULT_PORT, &config);
    ...
    if (SQCloudResultType(r) == RESULT_ROWSET) {
        ...
        for (uint32_t i=0; i<ncols; ++i) {
            uint32_t len;
            char *value = SQCloudRowsetColumnName(r, i, &len);
            printf("%.*s\n", len, value);
        }
        ...
        for (uint32_t i=0; i<nrows; ++i) {
            for (uint32_t j=0; j<ncols; ++j) {
                ...
                case VALUE_FLOAT: {
                    float value = SQCloudRowsetFloatValue(r, i, j);
                    printf("%f\n", value);
                ...
                case VALUE_TEXT: {
                    uint32_t len;
                    char *value = SQCloudRowsetValue(r, i, j, &len);
