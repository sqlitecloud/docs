...
If you want to register the managed database and enable tables programmatically instead of using the dashboard, see [CloudSync Management API](/docs/sqlite-sync-cloudsync-management-api). That flow uses a workspace-level management API key and is separate from the client authentication used by `cloudsync_network_set_apikey()` and `cloudsync_network_set_token()`.
